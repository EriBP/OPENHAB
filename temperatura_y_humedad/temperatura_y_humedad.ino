...
servidor de dirección IP ( 192 , 168 , 1 , 33 );
...
// Esta función permite tomar acciones en caso de que se reciba un mensaje correspondiente a un tema al cual se hará una suscripción
 devolución de llamada vacía ( char * tema, byte * mensaje, longitud int sin firmar ) { 
  ...
  if ( String (topic) == " esp32/output " ) {   // En caso de recibir un mensaje en el tema esp32/output
    si (mensajeTemp == " verdadero " ){
      De serie. println ( " Led encendido " );
      escritura digital ( flashLedPin , ALTO);
    } // fin del if (String(tema) == "esp32/salida")
    más  si (messageTemp == " false " ){
 ...
 reconexión vacía () {
  ...
  while (!cliente. conectado ()) { // Pregunta si hay conexión
    ...
    if (client. connect ( " ESP32CAMClient " )) { // Pregunta por el resultado del intento de conexión
      ...
      cliente. suscribirse ( " esp32/salida " ); // Esta función realiza la suscripción al tema
